...
export function useControls(selected: any[], props: { [x: string]: any }) {
    ...
    Object.keys(props).reduce(
      (acc, key) => ({
        ...
        [key]: {
          ...props[key],
          transient: false,
          onChange: (value: any, path: any, ctx: { initial: any }) =>
            !ctx.initial &&
            isFirst &&
            selected.length > 1 &&
            selected.forEach(
              (s: { setValueAtPath: (arg0: any, arg1: any) => any }, i: number) =>
                i > 0 && s.setValueAtPath(path, value)
            ),
          render: () => selected.length === 1 || selected.every((store: any) => store.getData()[key]),
        },
      }),
      {}
    ),
